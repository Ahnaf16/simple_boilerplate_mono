...
import 'package:core_functionality/src/app_const.dart';
...
Stream<T> failStream<T>(Failure f) => f.toStream<T>();
Future<T> failThrow<T>(Failure f) => f.toThrow<T>();
...
class Failure {
  ...
  Stream<T> toStream<T>() {
    return Stream<T>.error(this, stackTrace);
  }
